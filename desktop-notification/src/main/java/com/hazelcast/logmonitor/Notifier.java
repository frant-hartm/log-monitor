...
/**
 * Simple notification application which works as a HTTP server and uses system tray to display notifications.
 */
...
public class Notifier implements HttpHandler {
     ...
     * Construtor.
     * 
     * @param aPort port number to bind the HTTP server; if not positive number then the {@link #DEFAULT_PORT} is used
     * @param aDefaultIcon default icon file-name (without ".png" extension); if null then then the {@link #DEFAULT_ICON} is
     *        used
     * @param aSoundProperty desktop property name with the notification sound (as Runnable); if null then the
     *        {@link #DEFAULT_SOUND_PROP} is used
     * @throws IllegalStateException System tray is not supported in the OS by this java version.
     * @throws IllegalArgumentException default icon image is not available
     */
    ...
    /**
     * HTTP request/response handler. If the requested path points to an existing icon, then the system tray icon image is
     * changed and the notification message is retrieved from the request body. Empty response is returned with status code
     * either 200 (SC_OK - icon exists) or 404 (SC_NOT_FOUND - no such icon).
     * 
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        if (img != null) {
            ...
            br.close();
            String msgStr = sb.length() > 0 ? sb.toString() : null;
            String timeStr = DateFormat.getTimeInstance(DateFormat.MEDIUM).format(new Date());
            System.out.println(timeStr + " " + path + (msgStr != null ? " " + msgStr : ""));
     ...
     * 
     * @throws IOException HTTP server creation fails
     * @throws AWTException adding system tray icon fails
     */
    ...
    /**
     * Main. Reads arguments from the command line, creates notifier with provided (or default) parameters and starts the
     * notifier. If an {@link Exception} occures program is terminated with exit code -1.
     * 
     * @param args
     */
    ...
    /**
     * Tries to load PNG image with the given name (path) from the Notifier's classpath. Given name (path) is relative to the
     * Notifier's package. Leading slashes ('/') are removed from the name and suffix ".png" is appended.
     * 
